...
- **常驻后台**：关闭窗口后本地服务继续监听（托盘 / 菜单栏常驻），群里的链接才点得开
...
推送 `v*` 标签到 GitHub 仓库，`.github/workflows/build.yml` 会校验 tag 与四源版本一致后构建并创建 **Draft Release**，在 Release 页面下载产物：
...
所有命令支持 `--json`（stdout 只输出机器可读 JSON，诊断走 stderr）与统一退出码（0 成功 / 1 用法或配置错误 / 2 失败）。
